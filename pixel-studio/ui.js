...
const UI = (function () {
  ...
  function cacheEls() {
    var ids = [
      'sizeLabel', 'btnSize', 'btnImport', 'btnExport', 'btnHelp', 'saveState',
      'toolGrid', 'brushSelect',
      'tgMirrorX', 'tgMirrorY', 'tgShapeFill', 'tgPen', 'tgPressure',
      'zoomTip', 'preview', 'btnPlay', 'btnGrid2',
      'swatches', 'btnPalette', 'paletteGrid', 'colorInput', 'btnColorAdd', 'btnColorRemove',
      'btnPaletteClose', 'ovPalette',
      'frameList', 'frameCount', 'btnAddFrame', 'btnDupFrame', 'btnClearFrame', 'btnDelFrame',
      'fps', 'fpsVal', 'btnPlay2', 'btnOnion2', 'btnImport2', 'btnHelp2',
      'btnDrawer', 'drawerVeil',
      'ovSize', 'sizePresets', 'cusW', 'cusH', 'keepContent', 'btnSizeCancel', 'btnSizeOk',
      'ovImport', 'btnPickImage', 'btnPickProject', 'btnImportCancel', 'fileImport', 'fileProject',
      'ovExport', 'expFormats', 'expScales', 'gifOpts', 'gifLoop', 'gifTrans',
      'sheetOptRow', 'sheetCols', 'expProgress', 'expBar', 'btnExportCancel', 'btnExportOk',
      'ovResult', 'resultImg', 'resultHint', 'btnResultClose', 'btnResultSave',
      'ovHelp', 'btnHelpClose', 'toast'
    ];
  ...
  // ------------------------------------------------------------------
  function buildTools() {
    var html = CONFIG.tools.map(function (t) {
      return '<button class="icon-btn" data-tool="' + t.id + '" aria-pressed="false" ' +
        'aria-label="' + t.name + '（快捷键 ' + t.key + '）" title="' + t.name + ' · ' + t.key + '" style="--tc:' + t.tc + '">' +
        '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" ' +
        'stroke-linecap="round" stroke-linejoin="round">' + t.svg + '</svg>' +
        '<span class="kb">' + t.key + '</span></button>';
    }).join('');
    html += '<button class="icon-btn" id="btnUndo" aria-label="撤销" title="撤销 · Ctrl+Z" style="--tc:var(--blue)">' +
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round">' +
      '<path d="M4.5 10.5h10a4.5 4.5 0 0 1 0 9H9"/><path d="M8.5 6l-4 4.5 4 4.5"/></svg></button>';
    html += '<button class="icon-btn" id="btnRedo" aria-label="重做" title="重做 · Ctrl+Shift+Z" style="--tc:var(--blue)">' +
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round">' +
  ...
  function buildBrush() {
    el.brushSelect.innerHTML = CONFIG.brushSizes.map(function (n) {
      return '<option value="' + n + '">' + n + ' px</option>';
    }).join('');
  }

  function buildSizePresets() {
    var html = CONFIG.sizePresets.map(function (n) {
  ...
  function bindTop() {
    ...
    el.btnImport.addEventListener('click', function () { open(el.ovImport); });
    el.btnImport2.addEventListener('click', function () { document.body.classList.remove('side-open'); open(el.ovImport); });
    el.btnExport.addEventListener('click', function () { expCfg.scale = CONFIG.export.defaultScale; syncExportSheet(); open(el.ovExport); });
    el.btnHelp.addEventListener('click', function () { open(el.ovHelp); });
    el.btnHelp2.addEventListener('click', function () { document.body.classList.remove('side-open'); open(el.ovHelp); });
  }
  ...
  function bindTools() {
    el.toolGrid.addEventListener('click', function (e) {
      ...
      if (t) { App.setTool(t.dataset.tool); return; }
      if (e.target.closest && e.target.closest('#btnUndo')) { App.undo(); return; }
      if (e.target.closest && e.target.closest('#btnRedo')) { App.redo(); return; }
    });

    el.brushSelect.addEventListener('change', function () {
      State.setBrush(Number(el.brushSelect.value));
      App.invalidate();
    });
    ...
    el.btnOnion2.addEventListener('click', function () { State.toggleFlag('onion'); });
  }
  ...
  function bindToggle(btn, flag) {
    ...
    btn.addEventListener('click', function () {
      ...
      if (flag === 'penOnly') App.toast(v ? '手指将只用于平移画布' : '手指恢复绘制');
      if (flag === 'pressureOn') App.toast(v ? '笔压开始控制笔刷粗细' : '笔压已关闭，笔刷固定');
      App.invalidate();
    });
  }
  ...
  function bindPalette() {
    ...
    el.btnPalette.addEventListener('click', function () { open(el.ovPalette); });
    el.btnPaletteClose.addEventListener('click', function () { close(el.ovPalette); });
  ...
  function swatchHtml(hex, active) {
    return '<button class="swatch" data-hex="' + hex + '" style="--c:' + hex + '" ' +
      'aria-pressed="' + (active ? 'true' : 'false') + '" aria-label="颜色 ' + hex + '" title="' + hex + '"></button>';
  }
  ...
  function renderPalette() {
    ...
    if (quick.length < CONFIG.recentMax) {
      State.palette.forEach(function (h) {
        if (quick.length < CONFIG.recentMax && quick.indexOf(h) < 0) quick.push(h);
      });
    }
    el.swatches.innerHTML = quick.map(function (h) {
      return swatchHtml(h, h.toLowerCase() === cur);
    }).join('');
    // 完整色板放在浮层对话框里
    el.paletteGrid.innerHTML = State.palette.map(function (h) {
      return swatchHtml(h, h.toLowerCase() === cur);
    }).join('');
    el.colorInput.value = /^#[0-9a-fA-F]{6}$/.test(State.color) ? State.color : '#ff6b6b';
  }
  ...
  function bindFrames() {
    ...
    el.btnAddFrame.addEventListener('click', function () { App.addFrame(false); });
    el.btnDupFrame.addEventListener('click', function () { App.addFrame(true); });
    el.btnClearFrame.addEventListener('click', function () { App.clearFrame(); });
    el.btnDelFrame.addEventListener('click', function () { App.deleteFrame(); });
    ...
    el.frameList.addEventListener('dragstart', function (e) {
      ...
      if (e.dataTransfer) {
        ...
        try { e.dataTransfer.setData('text/plain', String(dragFrom)); } catch (err) { /* 忽略 */ }
      }
    });
  ...
  function renderFrames() {
    ...
    var html = '';
    for (var i = 0; i < n; i++) {
      html += '<button class="frame" data-i="' + i + '" draggable="true" ' +
        'aria-pressed="' + (i === State.current ? 'true' : 'false') + '" ' +
        'aria-label="第 ' + (i + 1) + ' 帧"><canvas></canvas><span class="idx">' + (i + 1) + '</span></button>';
    }
  ...
  function highlightFrame() {
    ...
    for (var i = 0; i < kids.length; i++) {
      kids[i].setAttribute('aria-pressed', i === State.current ? 'true' : 'false');
    }
    scrollFrameIntoView();
    updateHud();
  }
  ...
  function scrollFrameIntoView() {
    ...
    if (node && node.scrollIntoView) {
      try { node.scrollIntoView({ block: 'nearest', inline: 'nearest' }); } catch (e) { /* 忽略 */ }
    }
  }
  ...
  function bindAnim() {
    ...
    el.btnPlay.addEventListener('click', function () { App.togglePlay(); });
    el.btnPlay2.addEventListener('click', function () { App.togglePlay(); });
  }
  ...
  // 弹窗
  // ------------------------------------------------------------------
  function open(sheet) { if (sheet) sheet.hidden = false; }
  function close(sheet) { if (sheet) sheet.hidden = true; }

  function bindSheets() {
    [el.ovSize, el.ovImport, el.ovExport, el.ovResult, el.ovHelp, el.ovPalette].forEach(function (ov) {
      if (!ov) return;
    ...
    el.btnSizeCancel.addEventListener('click', function () { close(el.ovSize); });
    ...
    // --- 导入 ---
    el.btnPickImage.addEventListener('click', function () { el.fileImport.click(); });
    el.btnPickProject.addEventListener('click', function () { el.fileProject.click(); });
    el.btnImportCancel.addEventListener('click', function () { close(el.ovImport); });
    ...
    el.gifLoop.addEventListener('change', function () { expCfg.loop = el.gifLoop.checked; });
    el.gifTrans.addEventListener('change', function () { expCfg.trans = el.gifTrans.checked; });
    ...
    el.btnExportCancel.addEventListener('click', function () { close(el.ovExport); });
    el.btnExportOk.addEventListener('click', doExport);

    // --- 结果 ---
    el.btnResultClose.addEventListener('click', function () { close(el.ovResult); });
    ...
    // --- 手势 ---
    el.btnHelpClose.addEventListener('click', function () { close(el.ovHelp); });
  }
  ...
  function markPreset() {
    ...
    for (var i = 0; i < kids.length; i++) {
      ...
      if (!v || v === '0') { kids[i].setAttribute('aria-pressed', 'false'); continue; }
      var parts = v.split('x');
      var pw = Number(parts[0]), ph = Number(parts[1] || parts[0]);
      kids[i].setAttribute('aria-pressed', (pw === w && ph === h) ? 'true' : 'false');
    }
  }
  ...
  function syncExportSheet() {
    ...
    Array.prototype.forEach.call(el.expScales.children, function (b) {
      b.setAttribute('aria-pressed', Number(b.dataset.s) === expCfg.scale ? 'true' : 'false');
    });
  ...
  function onImagePicked() {
    ...
    if (!f) return;
    close(el.ovImport);
    Exporter.readImage(f).then(function () {
  ...
  function onProjectPicked() {
    ...
    if (!f) return;
    close(el.ovImport);
    Exporter.readProject(f).then(function () {
      App.toast('工程已恢复');
  ...
  function doExport() {
    ...
    Exporter.run(opts, function (p) {
      el.expBar.style.width = Math.max(0, Math.min(100, Math.round(p * 100))) + '%';
    }).then(function (res) {
      lastResult = res;
  ...
  function bindStateEvents() {
    ...
    State.on('frame', function () { highlightFrame(); markPreview(); });
    State.on('pixels', function () { updateThumb(); updateHud(); markPreview(); });
    State.on('size', function () { updateHud(); el.sizeLabel.textContent = State.w + '×' + State.h; markPreview(); });
    State.on('history', renderHistory);
    State.on('saving', function () { setSave('保存中…', false); });
  ...
  function renderToolState() {
    var cur = State.tool;
    Array.prototype.forEach.call(el.toolGrid.querySelectorAll('[data-tool]'), function (b) {
      b.setAttribute('aria-pressed', b.dataset.tool === cur ? 'true' : 'false');
    });
  }

  function renderBrushState() {
    el.brushSelect.value = String(State.brush);
  }
  ...
  function updateHud() {
    ...
    el.zoomTip.innerHTML = '<b>' + View.zoomLabel() + '</b> · ' + State.w + '×' + State.h +
      ' · 帧 ' + (State.current + 1) + '/' + State.frameCount +
  ...
  // 预览窗
  // ------------------------------------------------------------------
  function markPreview() { previewDirty = true; }
