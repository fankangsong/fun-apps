...
| 飞船耐久 | **碰撞一次立即爆炸消失**（`shipHP = 1`，像素爆炸销毁后补位。2026-08-28 需求变更：原 3 点血设计改为即撞即爆，机制保留便于回调） |
...
- 每艘 `shipHP`（**1**，2026-08-28 变更）点血；船身舷窗像素即血条（亮 = 有血）。
- 受击即爆炸：球撞飞船当帧完成变向后，飞船像素爆炸（见 §6.2）、销毁刚体、
  释放线路并触发补位计时（`refillDelay`）。
...
- 13×6 字符画矩阵，`visualScale`（**8px**，2026-08-28 放大 2 倍）每像素 → 视觉 104×48；
  物理矩形略小：`physW × physH` = 96 × 40（宽容度）。
...
// debug 组新增调试参数：shipMax（null = 用 space.maxShips，可设场上飞船数量上限）
